fn main() {
    ...
    // Exercise n°
    // create the longest function that takes as input two reference of type str and returns the longest str as a reference
    // First, try to compile the code without lifetime and look at the error message.
    // Then, use lifetimes in order for the code to compile.

    // Exercise n°12
    // create the longest_mut function that is similar to the longest function. The difference is that the two input are mutable as well as the  output as well
}
